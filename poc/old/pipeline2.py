...
class MakePipeline():
    ...
    def __init__(self, arch, optimize, metric):
        self.arch = arch
    ...
    def evaluate(self, X, y):
        self = MakePipeline('sk', False, 'mae')
        ...
        cfg = self._merge_usser_dict_connector(
            cfg, connector)

        for c in cfg:
            print(c)

            cfg_t = c.loc[c['block'] != 'models'][['name', 'clas', 'values']]
            cfg_m = c.loc[c['block'] == 'models'][['name', 'clas', 'values']]

            for t in cfg_t.iterrows():
                print(t[1])
...
# słownik
# pociąć słownik
# dla transformera sfitować transformery
# dla modelu sfitować modele
# jeżeli sieć to rób funkcję sieć a nie tak jak do tej pory kod
# jeżeli nie sieć to rób funkcję nie sieć
